...
void addOneLongVector(){
	...
	size_t threadsPerBlock = 256;
	size_t numberOfBlocks = numberOfSMs * 32;
	init<<<numberOfBlocks, threadsPerBlock>>>(data, N*100);
	init<<<numberOfBlocks, threadsPerBlock>>>(&data[N*100], N*100);
	cudaDeviceSynchronize();
	addVector<<<numberOfBlocks, threadsPerBlock>>>(data,&data[N*100],&data[N*2*100], N);
